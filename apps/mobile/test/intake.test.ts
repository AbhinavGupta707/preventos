...
const answeredSmoking = () => {
  ...
  // HSI answers are canonical option INDICES: ttfc idx 0 = "within 5 min" (score 3),
  // cpd idx 2 = "21 to 30" (score 2) → total 5 (high). Scores live only in the
  // instrument definition; the app never duplicates them.
  s = intakeReducer(s, { type: "answer_hsi", item: "hsi-ttfc", value: 0 });
  s = intakeReducer(s, { type: "answer_hsi", item: "hsi-cpd", value: 2 });
...
describe("toBfoSection", () => {
  it("produces a BfoSection with HSI instrument score from @preventos/instruments scoring", () => {
    ...
    if (result.ok) {
      ...
      expect(result.value.instrumentScores["hsi"]).toBe(5);
