...
import { HSI, assertServable } from "@preventos/instruments";
...
export default function SmokingIntake() {
  ...
  // Invariant 2 gate: an instrument without cleared licensing never renders.
  const servable = useMemo(() => assertServable(HSI), []);
  if (!servable.ok) {
    return (
      <Screen>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {step === "hsi1" ? (
          <View testID="intake-step-hsi1">
            ...
            {HSI.items[0]!.options.map((opt) => (
              <OptionRow
                key={opt.label}
                testID={`hsi-ttfc-${opt.value}`}
                label={opt.label}
                selected={intake.answers.hsi["hsi-ttfc"] === opt.value}
                onPress={() => {
                  setIntake(intakeReducer(intake, { type: "answer_hsi", item: "hsi-ttfc", value: opt.value }));
                  advance();
                }}
              />
            ))}
        ...
        {step === "hsi2" ? (
          <View testID="intake-step-hsi2">
            ...
            <View style={{ height: space.md }} />
            {HSI.items[1]!.options.map((opt) => (
              <OptionRow
                key={opt.label}
                testID={`hsi-cpd-${opt.value}`}
                label={opt.label}
                selected={intake.answers.hsi["hsi-cpd"] === opt.value}
                onPress={() => {
                  setIntake(intakeReducer(intake, { type: "answer_hsi", item: "hsi-cpd", value: opt.value }));
                  advance();
                }}
              />
            ))}
