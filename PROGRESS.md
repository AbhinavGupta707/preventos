...
| WS2 | Mobile app (2.1–2.6) (wave 2) | in_review | wp/ws2-mobile-app | Design system (calm/non-clinical tokens), ≤90s smoking intake → BFO (HSI verbatim via new `@preventos/instruments`), unified today surface (one NBA card via typed `ApiPort`+mock), rescue 1-tap everywhere (offline: Release bundle embedded, no network in rescue path), plans+cross-programme savings+days-won (reuses `decisions.daysWon`), push choreography (primer→OS, denial terminal), coach shell (streaming reducer + deterministic pre-LLM crisis gate → scripted crisis screen). 31 mobile + 12 instrument tests; Maestro golden-path + crisis-gate flows green on iOS sim Release build; EAS build `preview-simulator` profile. Assumptions: StyleSheet tokens not NativeWind (RN 0.85 compat risk); Steady enrolment gated off until AUDIT+hard-stop (inv. 4); sleep intake bespoke Qs (SCI blocked, licensing); EAS device internal-distribution build needs owner Apple creds |
