...
| 4.2m | Content pipeline + canonical migration (WP4.2 + WP4.4) | **done** | wp/4.2m-content-pipeline | Validator CLI (`pnpm content:validate`) in `pnpm verify`, per-pack; claims lint consumes WP10.10 register (all vectors enforced; live injection of "treats insomnia" proven to exit 1); sign-off schema enforced (non-draft atoms need registry entry); FK reading-age scorer (warn-only); schema vocab +7 types, strict mode; all 4 packs migrated (149+45+91+54 atoms), prose preserved, LEGACY_PACKS ratchet EMPTY, one merged catalog; `@preventos/instruments`: HSI/AUDIT-C/TTFV score published vectors, hash-locked verbatim items (tamper fails), SCI registered blocked-pending-license + proven unreachable. Assumptions: vaping/alcohol BCT+COM-B tags are DRAFT pending clinical review; vaping ids renamed; PHQ-2/GAD-2 registry entries deferred |
